...
Инструмент для пакетного обновления session-cookies **личных** Google-аккаунтов через Playwright. Для каждого аккаунта имитируется «человеческий» повторный логин (ввод пароля, при необходимости — подтверждение через recovery-email), после чего обновлённые cookies складываются в локальный файл и в Back4App.

> 🙏 **О проекте.** Все аккаунты, с которыми работает этот скрипт, — **мои личные**. Я создал их **вручную, на выдуманные имена и фамилии** (исключительно для собственных нужд) и использую исключительно в **благих, законных и личных целях**: тестирование собственных сервисов, обучение автоматизации, личные эксперименты с Parse/Back4App, проверка собственных интеграций. Никакого спама, рассылок, парсинга чужих данных, обхода чужих систем или иной деятельности, нарушающей законы или ToS Google, не осуществляется. Скрипт применяется только к аккаунтам, владельцем которых являюсь я сам.
>
> Если вы форкаете или копируете проект — пожалуйста, используйте его **только для своих собственных, заранее созданных вами лично аккаунтов** и только в легальных целях. Автор не несёт ответственности за использование кода третьими лицами.
...
## 🎯 Назначение
Массовое обновление session-cookies для большого числа **моих собственных, лично созданных вручную** Google-аккаунтов на выдуманные имена. Используется, когда:
- нужно «освежить» сессии в своих собственных аккаунтах, не заходя в каждый руками;
- cookie-файлы истекли/частично потеряли актуальность, и Google требует повторный ввод пароля;
- сессии хранятся в Back4App и используются моими другими личными сервисами (для собственных тестовых интеграций, обучения, экспериментов).
...
> Аккаунты — **исключительно мои, зарегистрированные мной лично, на вымышленные ФИО**, и используются строго в **благих и личных целях** (самообучение автоматизации, проверка собственных pet-проектов, отладка связки Playwright + Back4App и т.п.). Никакой коммерческой, спам- или противоправной активности.
...
- **Один общий пароль и набор recovery-email на все аккаунты.** Зашиты в `src/tasks/auth.ts`, см. `12Qwert34` / `iwocop@gmail.com` / `kupianas@gmail.com`. Это удобно для личных батч-прогонов, но при необходимости можно параметризовать.
- **Примитивный stealth.** Переписывается только `navigator.webdriver` + `navigator.vendor` + фиксированный User-Agent. Современный bot-detection Google смотрит на canvas, WebGL, шрифты, TLS-fingerprint и т.д. — при больших объёмах прогона возможны капчи и дополнительные проверки.
- **Одинаковый технический профиль на все аккаунты.** Один и тот же User-Agent, viewport, fingerprint и т.п. — чем больше аккаунтов проходит с одного IP, тем выше шанс срабатывания антибот-защит. Для личных небольших объёмов это не проблема, но при прогоне тысяч аккаунтов подряд стоит добавить ротацию.
...
4. **Используйте только на собственных аккаунтах.** Этот проект задуман и применяется исключительно для **моих личных аккаунтов**, которые **я сам создал вручную, на выдуманные имена**, и которые использую **в благих, законных и личных целях**. Скрипт не предназначен и не должен использоваться для:
   - входа в чужие аккаунты;
   - массовых рассылок, спама, парсинга;
   - обхода защит чужих систем;
   - любой деятельности, нарушающей законы вашей юрисдикции или ToS Google.
   
   Применение к чужим аккаунтам — нарушение закона и правил Google, и автор проекта не имеет к этому никакого отношения.

5. **Скрипт не отключает и не обходит двухфакторную аутентификацию.** Используется тот же пароль и резервный email, что и при обычном ручном входе; дополнительных методов обхода 2FA не реализовано.
